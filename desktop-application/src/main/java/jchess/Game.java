...
public class Game extends JPanel implements MouseListener, ComponentListener
{
    ...
    Game()
    {
        ...
        // --- FIX: ensure board layout initializes before first resize ---
        SwingUtilities.invokeLater(() -> {
        int height = this.getHeight() >= this.getWidth() ? this.getWidth() : this.getHeight();
        if (height <= 0) height = chessboard.get_height(false);
        int chess_height = (int) Math.round((height * 0.8) / 8) * 8;
        chessboard.resizeChessboard(chess_height);
    ...
    static public void loadGame(File file)
{
    ...
    Game newGUI = Game.createLoadedLocalGame(whiteName, blackName, tempStr);
}

/**
 * Factory method for a local loaded game.
 * Encapsulates how a Game is created and configured from player names and PGN text.
 */
static Game createLoadedLocalGame(String whiteName, String blackName, String movesText) {
    Game game = JChessApp.jcv.addNewTab(whiteName + " vs. " + blackName);
    Settings settings = game.settings;

    // Use Settings helper where possible
    settings.configurePlayers(whiteName, blackName);
    settings.setGameMode(Settings.gameModes.loadGame);
    settings.setGameType(Settings.gameTypes.local);

    game.newGame();
    game.blockedChessboard = true;
    game.moves.setMoves(movesText);
    game.blockedChessboard = false;
    ...
    public void newGame()
    {
        ...
        // --- FIX: ensure markers/selection are initialized on first game ---
        // Force initialization of selection markers
        int height = this.getHeight() >= this.getWidth() ? this.getWidth() : this.getHeight();
        if (height <= 0) {                      // first render may report 0
            height = chessboard.get_height(false);
        }
        int chess_height = (int) Math.round((height * 0.8) / 8) * 8;
        chessboard.resizeChessboard(chess_height);

        // clear any stale selection state so marker overlay starts clean
        chessboard.unselect();
        // force layout + paint now (otherwise markers only appear after opening a 2nd game)
        chessboard.revalidate();
        chessboard.repaint();
        this.revalidate();
        this.repaint();
...
private void handleLeftClick(MouseEvent event) {
    if (!blockedChessboard) {
        try {
            ...
            else if (chessboard.activeSquare == sq) {
                chessboard.unselect();
            }
            else if (chessboard.activeSquare != null && chessboard.activeSquare.piece != null
                    && chessboard.activeSquare.piece.allMoves().indexOf(sq) != -1) {
                handleMoveToSquare(sq);
            }
...
/** Extracted method for "actually perform the move + check checkmate". */
private void handleMoveToSquare(Square targetSquare) {
    if (settings.gameType == Settings.gameTypes.local) {
        chessboard.move(chessboard.activeSquare, targetSquare);
    }
    else if (settings.gameType == Settings.gameTypes.network) {
        client.sendMove(
                chessboard.activeSquare.pozX,
                chessboard.activeSquare.pozY,
                targetSquare.pozX,
                targetSquare.pozY
        );
        chessboard.move(chessboard.activeSquare, targetSquare);
    ...
    King king = (this.activePlayer == settings.playerWhite)
            ? chessboard.kingWhite
            : chessboard.kingBlack;
